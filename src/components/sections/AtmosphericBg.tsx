...
export function AtmosphericBg() {
  ...
  useEffect(() => {
    ...
    const handleMouse = (e: MouseEvent) => {
      ...
      glowRef.current.style.background =
        `radial-gradient(circle 700px at ${x}% ${y}%, rgba(201,164,90,.09) 0%, transparent 70%)`;
    };
  ...
  return (
    <>
      ...
      {/* Orb 1 */}
      <div aria-hidden className="fixed rounded-full pointer-events-none" style={{ zIndex: 3, width: 440, height: 440, background: "rgba(201,164,90,.1)", filter: "blur(90px)", mixBlendMode: "screen", top: -80, left: -120, animation: "orb1 22s ease-in-out infinite" }} />
      {/* Orb 2 */}
